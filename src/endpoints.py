        ...
        async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
            ...
            for line in raw_manifest_lines:
                ...
                if clean_line and not clean_line.startswith("#"):
                    ...
                    # Forces child tracks back through the /anime prefix module to align with Vercel mapping
                    if '.m3u8' in clean_line:
                        proxied_track_line = f"/anime/proxy_m3u8?{urlencode(query_params)}"
                    else:
                        proxied_track_line = f"/anime/proxy_segment?{urlencode(query_params)}"
